...
def get_kitti_image_info(path,
    ...
    def map_func(idx):
        image_info = {'image_idx': idx, 'pointcloud_num_features': 4} # original code
        annotations = None
        if velodyne:
            image_info['velodyne_path'] = get_velodyne_path(
                idx, path, training, relative_path)
        # image_info['img_path'] = get_image_path(idx, path, training, # original code
        #                                         relative_path) # original code
        # if with_imageshape:  # original code
        #     img_path = image_info['img_path'] # original code
        #     if relative_path: # original code
        #         img_path = str(root_path / img_path) # original code
        #     image_info['img_shape'] = np.array( # original code
        #         io.imread(img_path).shape[:2], dtype=np.int32) # original code
        if label_info:
            label_path = get_label_path(idx, path, training, relative_path)
            if relative_path:
                label_path = str(root_path / label_path)
            annotations = get_label_anno(label_path)
        # if calib: # original code
        ...
        #     image_info['calib/Tr_imu_to_velo'] = Tr_imu_to_velo # original code
...
def remove_dontcare(image_anno):
    ...
    relevant_annotation_indices = [
        i for i, x in enumerate(image_anno['name']) if x != "0"
    ]
    for key in image_anno.keys():
        img_filtered_annotations[key] = (
...
def filter_kitti_anno(image_anno,
    ...
    if '`0' in used_classes and dontcare_iou is not None:
        dont_care_indices = [
...
def add_difficulty_to_annos(info):
    # min_height = [40, 25, # original code
    ...
    #     else:
    #         diff.append(-1) 
    # annos["difficulty"] = np.array(diff, np.int32) # original code
    annos["difficulty"] = np.zeros( (len(dims), 1) )
